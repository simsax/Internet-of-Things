...
String senMlEncode3() {
  doc2.clear();
  doc2["deviceID"] = "Yun_led";
  doc2["RESTuri"] = "/yun/led";
  doc2["MQTTtopic"] = "/tiot/8/led";
  doc2["resource"] = "Led";
...
void loop() {  
  ...
  String message = senMlEncode2(); //labsw3
  mqtt.publish(my_base_topic + String("/devices"), message);

  message = senMlEncode3(); //labsw3
  mqtt.publish(my_base_topic + String("/devices"), message);
  
  message = senMlEncode(calcTemp());
